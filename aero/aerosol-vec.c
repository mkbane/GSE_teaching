...
int main(int argc, char* argv[]) {
  ...
  //MMU  double start=omp_get_wtime();   // we make use of the simple wall clock timer available in OpenMP
  ...
  //MMU output_particles(x,y,z, vx,vy,vz, gas, liquid, num);
  ...
  */
  ...
  for (time=1; time<=timesteps; time++) {
    ...
    for(i=0; i<num; i++) {
      ...
      //#pragma omp simd private(dx, dy, dz, temp_d, d, F, ax, ay, az)
      //VEC:      for (j=0; j<num; j++) {
#pragma loop ivdep
      for (j=0; j<i; j++) {
        //VEC: if (j != i) {
...
#pragma loop ivdep      
  ...
  //MMU  printf("Time to init+solve %d molecules for %d timesteps is %g seconds\n", num, timesteps, omp_get_wtime()-start);
  printf("Time to init+solve %d molecules for %d timesteps is %g seconds\n", num, timesteps, get_wtime()-start);
